...
const channelShareData = [
  { label: "API", value: 34, color: chartPalette.blue },
...
function DonutChartCard() {
  ...
  return (
    <Card data-component-id="CHART-07" depth="base" id="chart-07-donut">
      <CardHeader action={<ComponentIdBadge id="CHART-07" />}>
...
function DonutTooltip({
  ...
  return (
    <div className="rounded-[6px] border border-[var(--ds-gray-alpha-400)] bg-[var(--ds-background-100)] p-2 text-[11px] shadow-sm">
      <p className="font-mono text-[var(--ds-gray-700)]">{item.label}</p>
      <p className="mt-1">
        Share: <strong>{Math.round((item.value / total) * 100)}%</strong>
      </p>
    </div>
  );
}
